...
{
	// Default implementation returns the current status as stored in m_ServiceStatus.
	ReportStatusToSCM();
}
...
void CNTService::OnUserDefinedRequestEx(DWORD dwControl, DWORD dwEventType, LPVOID lpEventData, LPVOID lpContext)
{
	TRACE(_T("CNTService::OnUserDefinedRequestEx was called.\n"));
	// Default implementation just do nothing.
}

void CNTService::ServiceCtrlHandler(DWORD dwControl)
{
	// Just switch on the control code sent to us and call the relevent virtual function.
	switch (dwControl)
	{
...
DWORD CNTService::ServiceCtrlHandlerEx(DWORD dwControl, DWORD dwEventType, LPVOID lpEventData, LPVOID lpContext)
{
	// Just switch on the control code sent to us and call the relevent virtual function.
	switch (dwControl)
	{
		...
		default:
			OnUserDefinedRequestEx(dwControl, dwEventType, lpEventData, lpContext);
			break;
	}
...
BOOL CNTService::RegisterCtrlHandler()
{
	...
	if (m_hStatus == 0)
		TRACE(_T("Failed in call to RegisterServiceCtrlHandler in RegisterCtrlHandler! ec: %d\n"), ::GetLastError());
...
BOOL CNTService::RegisterCtrlHandlerEx()
{
	...
	m_hStatus = ::RegisterServiceCtrlHandlerEx(m_sServiceName, _ServiceCtrlHandlerEx, 0);
	if (m_hStatus == 0)
		TRACE(_T("Failed in call to RegisterServiceCtrlHandlerEx in RegisterCtrlHandlerEx! ec: %d\n"), ::GetLastError());
...
{
	// Default implementation does nothing but asserts, your version should call RegisterCtrlHandler plus implement its own service specific code.
	ASSERT(FALSE); 
}

void CNTService::_ServiceCtrlHandler(DWORD dwControl)
{
	// Convert from the SDK world to the C++ world. In this implementation we just use a single static, In the future
	// we could use a map just like MFC does for HWND to CWnd conversions.
...
DWORD CNTService::_ServiceCtrlHandlerEx(DWORD dwControl, DWORD dwEventType, LPVOID lpEventData, LPVOID lpContext)
{
	...
	// we could use a map just like MFC does for HWND to CWnd conversions.
	ASSERT(sm_lpService != NULL);
	return sm_lpService->ServiceCtrlHandlerEx(dwControl, dwEventType, lpEventData, lpContext);
}
