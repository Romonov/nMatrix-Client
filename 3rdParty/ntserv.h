...
// You are meant to derive your own class from this and override its functions to implement your own service specific functionality.
class CNTService
{
public:
	...
	// Persistance support: Allows saving and restoring of a services settings to the
	...
	virtual DWORD WINAPI ServiceCtrlHandlerEx(DWORD dwControl, DWORD dwEventType, LPVOID lpEventData, LPVOID lpContext);
	...
	// Kicks off the Service.
	// You would normally call this some where in your main/wmain or InitInstance a standard process rather than as a service.
	// If you are using the CNTServiceCommandLineInfo class, then internally it will call this function for you.
	virtual BOOL Run();
...
protected:
	...
	// These two static functions are used internally to go from the SDK functions to the C++ member functions.
